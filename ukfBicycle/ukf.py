...
def move(x, dt, wheelbase=2.5, u=np.array([0.,0.]), eps=1e-5):
    ...
    if abs(steering_angle) > eps:  # non-zero steering: turning
        ...
        return x + np.array([distance_traveled * cos(hdg), distance_traveled * sin(hdg), acc * dt, 0])
...
class Ukf(object):
    ...
    def __init__(self, dt=0.1, wheelbase=2.5, startx=0., starty=0., startspeed=0., starthdg=0., stdx=1.0, stdy=1.0, noise=0.1):
        """
        initialize Ukf
        :param dt: dt
        ...
        self.ukf.P = np.array([[ 4.78559217e-01, -1.94838441e-03,  2.48955909e-01, -1.25313018e-03],
                               ...
                               [-1.25313018e-03,  1.83459578e-01,  6.06738360e-06,  2.18001626e-01]])

        self.ukf.R = np.diag([stdx**2, stdy**2])
        self.ukf.Q = np.eye(4)*noise
