...
\begin{abstract}
  ...
  %  We propose to train a policy using a learned model of the environment dynamics while explicitly penalizing the mismatch between these two distributions over a fixed time horizon,
...
\begin{figure*}[t!]
    \centering
    \subfigure{\includegraphics[width=0.65\textwidth]{figures/driving/ep_uncertainty_cost-crop.pdf}} \\
...
%If the dataset $\mathcal{D}$ consists of \textit{expert} trajectories, a natural choice would be to set $C(\hat{s}_{t+i}) = U(\hat{s}_{t+i}) = \frac{1}{2} \ell(\hat{s}_{t+k}, s_{t+k})$. This give a model-based imitation learning objective which simultaneously optimizes the policy cost and the uncertainty cost.
...
%Although this leads to a set of states which the model is presumably confident about, it may not be a trajectory which also satisfies the policy cost $\textit{C}$ unless the dataset consists of \textit{expert} trajectories.
...
\begin{figure*}[t!]
  ...
  \subfigure[]{
    \includegraphics[width=0.42\textwidth]{figures/driving/rollout_length_distance-crop.pdf}
    \includegraphics[width=0.42\textwidth]{figures/driving/rollout_length_success-crop.pdf}
    }
    \subfigure[]{
...
%Details of the training procedure and policy cost used are given in Appendix \ref{training-details-appendix}. 
...
%this becomes the negative of our loss term in Equation \ref{model-loss}: a variational autoencoder trained with dropout. 
...
\begin{figure*}[h]
  ...
  \begin{tabular}{|llrr|}
    ...
    \hhline{|====|}
    \modelnamedrop + $z$-dropout & $z_t \sim p(z)$ & 168.2 & 72.1 \\
    \modelnamedrop & $z_t \sim p(z)$ & 157.4 & 63.6 \\
    \modelnamedrop + $z$-dropout & $z_t \sim q_\phi(z | s_{1:t}, s_{t+1})$ & 126.1 & 30.8 \\
    \modelnamedrop & $z_t \sim q_\phi(z | s_{1:t}, s_{t+1})$ & 86.0 & 19.2 \\
    \hline
  \end{tabular}
    }
    \label{action-sens}
