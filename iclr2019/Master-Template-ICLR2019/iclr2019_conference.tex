...
\title{Learning Policies from Observational Data with Learned Environment Models}
...
%  We first learn a dynamics model from a large dataset of human driving trajectories, which is then used to train a fast, feedforward policy network.
  Learning a policy using only observational data is challenging because the distribution of states induced by a learned policy may differ from the distribution of states observed during training.
  In this work, we propose a general framework for learning policies from purely observational data using learned environment models.
  Policies are trained by minimizing two costs: one cost which measures the optimality of the state distribution which it induces, and one cost which measures how well this distribution matches the distribution seen during training.
...
This is illustrated in Figure \ref{svg}. Which method to apply depends on whether the observational data contains expert trajectories and the availability of a well-defined cost function.
    ...
    \begin{align*}
    \underset{\psi}{\mbox{ argmin }} \Big[ \sum_{i=1}^{T} C(s_{t+i}) + U(s_{t+1}) \Big],  \mbox{ such that: }
    \begin{cases}
      \tilde{a}_{t+i} = \pi_\psi(s_{t+i-1}) \\
      \tilde{s}_{t+i} = f(\tilde{s}_{t+i-1}, \tilde{a}_{t+i}) \\
      \end{cases}
    \end{align*}
