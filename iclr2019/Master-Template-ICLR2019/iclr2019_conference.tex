...
\begin{abstract}
%  In this work, we propose two model-based approaches for learning policies for autonomous driving from purely observational data.
%  We first learn a dynamics model from a large dataset of human driving trajectories, which is then used to train a fast, feedforward policy network.
  ...
  We evaluate our approach using a large-scale observational dataset of driving behavior recorded from traffic cameras, and show that we are able to learn effective driving policies from purely observational data, with no environment interaction or queries of an expert.
...
This objective function contains two terms: a \textit{policy cost} $C$, which reflects the underlying objective the policy is trying to learn, and an \textit{uncertainty cost} $U$, which reflects how close the predicted state induced by the policy network is to the manifold which the data $\mathcal{D}$ is drawn from:
    ...
    \end{align*}
        ...
        We propose here a general form based on the uncertainty of the dynamics model. 
        ...
        Here, we use dropout as a means to calculate this model uncertainty.
...
\end{figure*}
  ...
  For the stochastic model, this requires inferring the latent variables corresponding to the training set trajectory using the posterior network $q_\phi$.
