...
\begin{align}
  \label{eq:update-eqn}
  \mathcal{L}(s_{1:t}, s_{t+1}, a_t) = \|s_{t+1} - f_\theta(s_{1:t}, a_t, z_t) \|_2^2 + \beta D_{KL}(\mathcal{N}(\mu_\phi, \sigma_\phi) || p(z))
\end{align}
...
In our case, we are training an action-conditional video prediction model which we will later use to train a policy. This leads to an additional requirement: it is important for the prediction model to accurately respond to input actions, and not use the latent variables to encode factors of variation in the outputs which could be explained by the actions.
To this end we propose to use a mixture of two Gaussians, with one component fixed to the prior:
...
\begin{alignat}{2}
  ...
  &u &&\sim \mathcal{B}(p_u) \\
  &z_t &&\sim (1-u) \cdot \mathcal{N}(\mu_t, \sigma_t) + u \cdot p(z)
\end{alignat}

This is related to the $\textit{dropout approximating distribution}$ described in \citep{Gal16}, and can be seen as applying a form of dropout to the latent variables at training time: with probability $p_u$, the latent variable is sampled from the fixed prior, rather than the learned posterior. \footnote{If the variances of both prior and posterior are fixed to zero, this reduces to standard dropout.}  
This forces the prediction model to extract as much information as possible from the input states and actions by masking the latent variable a fraction of the time. In our experiments we will refer to this parameterization as $z$-dropout. 
...
Our approach can be viewed as training a Bayesian neural network (BNN) \citep{Neal1995} with latent variables using variational inference \citep{Jordan1999}. 
...
\begin{align}
  q_\eta(z, \theta) = \mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) \cdot q_{\theta^*}(\theta)
\end{align}

Here $q_\phi$ represents a posterior network with parameters $\phi$ which outputs the mean and variance of a diagonal Gaussian over latent variables.
...
\begin{align*}
D_{KL}(q_\eta(\theta, z) || p_0(\theta, z)) &= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) \cdot q_{\theta^*}(\theta) || p_0(z) \cdot p_0(\theta)) \\
&= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) + D_{KL}(q_{\theta^*}(\theta | z) || p_0(\theta | z))  \\
&= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) + D_{KL}(q_{\theta^*}(\theta) || p_0(\theta))  \\
\end{align*}
%\begin{align}
%  \mathcal{L}(\eta; s_{1:t}, s_{t+1}, a) = \int p(s_{t+1} | f_\theta(s_{1:t}, a, z)) q_\eta(\theta, z) d\theta dz + D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) + 
...
\begin{align}
  \mathcal{L}_{VLB}(\theta^*, \phi; s_{1:t}, s_{t+1}, a) &\approx -\log p(s_{t+1} | f_{\bar{\theta}}(s_{1:t}, a, z)) + D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) 
\end{align}

where $\bar{\theta} \sim q_{\theta^*}(\theta)$ and $z \sim q_\phi(s_{1:t}, s_{t+1}))$. Optimizing this variational lower bound is equivalent to training a Variational Autoencoder with dropout.
...
%Both approaches aim to model aleatoric and epistemic uncertainty
%They did so in the context of BNNs trained using $\alpha$-divergences, which they found to outperform Gaussian Processes and unregularized model-based policy learning on low-dimensional problems.
%Both our approach and theirs aim to model aleatoric and epistemic uncertainties, but do so in different ways and have complementary strengths.
$\alpha$-BNNs have been shown to sometimes yield better uncertainty estimates than variational inference-based methods \citep{}, which can underestimate model uncertainty by fitting to a local mode of the exact posterior. However, they also require computing multiple samples from the distribution over model weights when training the forward model, which increases memory requirements and limits scalability to high-dimensional settings such as the ones we consider here. 
...
\end{figure*}
  ...
  \begin{tabular}{|lll|ll|}
    \hline
    Method & Model & Uncertainty Reg. & Mean Distance & Success Rate (\%)  \\
    \hhline{|=====|}
    Human & & & $209.4$ & $100.0$ \\
    \hline
    No action & & & $87.3$ & $16.2$ \\
...
Figure \ref{performance} compares performance for the different methods, where MPUR and MPER are optimized over rollouts lengths in $\{1, 3, 5, 10, 20, 30, 40\}$ (we will later compare the effect of rollout length). 
...
The stochastic model trained with a standard Gaussian posterior yields some improvement over the deterministic model, but this difference is relatively minor.
...
Figure \ref{rollout-length} shows the performance of \modelnamedrop and \modelnameil for different rollout lengths.
...
%    \subfigure[without uncertainty cost]{\includegraphics[width=0.48\textwidth]{figures/driving/svg_no_reg_prediction-crop.pdf}} \\
  ...
  \begin{tabular}{|lrrrr|}
    ...
    \hhline{|=====|}
    VG & 97 & 0.0 & 0.03 & 4087.4 \\
    \modelnamedrop & 962 & 56.4 & 0.16 & 0.3 \\
    \hline
  \end{tabular}
    }
