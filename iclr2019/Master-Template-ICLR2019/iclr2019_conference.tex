...
Our approach can be viewed as training a Bayesian neural network (BNN) with latent variables using variational inference. 
The predictive distribution for $s_{t+1}$ is given by
...
We instead approximate it with the variational distribution:

\begin{align}
  q_\eta(z, \theta) = \mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) \cdot q_D(\theta)
\end{align}
...
The distribution $q_D$ is the \textit{dropout approximating distribution} over model parameters described in \citep{Gal16} (Section 3.2 of Supplement). 
This defines a mixture of two Gaussians with small variances over the rows of each weight matrix in the forward model, with the mean of one Gaussian fixed at zero. The parameters of this distribution are the model weights, and samples can be drawn by applying different dropout masks. The parameters of the variational distribution are thus $\eta = \{ \theta^*, \phi \}$, 
...
\begin{align}
  \mathcal{L}(\eta; s_{1:t}, s_{t+1}, a) &= -\int \log p(s_{t+1} | f_\theta(s_{1:t}, a, z)) q_\eta(\theta, z) d\theta dz +  D_{KL}(q_\eta(\theta, z) || p_0(\theta, z))
  \label{elbo}
\end{align}

Here $p_0(z, \theta) = p_0(z) \cdot p_0(\theta)$ represents a prior over latent variables and model parameters. By applying the chain rule for KL divergences, we obtain:

\begin{align*}
D_{KL}(q_\eta(\theta, z) || p_0(\theta, z)) &= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) \cdot q_D(\theta) || p_0(z) p_0(\theta)) \\
&= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) + D_{KL}(q_D(\theta | z) || p_0(\theta | z))  \\
&= D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) + D_{KL}(q_D(\theta) || p_0(\theta)  \\
\end{align*}
%\begin{align}
...
\begin{align}
  \mathcal{L}(\theta^*, \phi, (s_{1:t}, s_{t+1}, a)) &\approx -\log p(s_{t+1} | f_{\bar{\theta}}(s_{1:t}, a, z)) + D_{KL}(\mathcal{N}(z | q_\phi(s_{1:t}, s_{t+1})) || p_0(z)) 
\end{align}

where $\bar{\theta} \sim q_D(\theta)$ and $z \sim q_\phi(s_{1:t}, s_{t+1}))$. Optimizing this variational lower bound is equivalent to training a Variational Autoencoder with dropout.

Once this model is trained, we obtain an approximate distribution over outputs $p(\hat{s}_{t+1} | s_{1:t}, a)$ which we can sample from by sampling from the approximate posterior.
We now show that the covariance of the outputs $\hat{s}_{t+1}$ can be decomposed into a sum of two covariance matrices which represent the aleatoric and epistemic uncertainty.
...
The first term is the covariance of the random vector $\mathbb{E}_z[\hat{s}_{t+1} | s_{1:t}, a, \theta]$ when $\theta \sim q_D(\theta)$.
...
The second term represents the covariance of the predictions obtained by sampling different latent variables $z \sim q_\phi(s_{1:t}, s_{t+1})$ averaged over different dropout masks, and ignores any contribution to the variance from $\theta$. As such it represents the aleatoric uncertainty.
Our uncertainty penalty explicitly penalizes the trace of the first matrix where the expectation over $z$ is approximated by a single sample from the posterior network. 
