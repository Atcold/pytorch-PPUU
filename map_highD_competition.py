...
# Recording meta data
def read_recoding_meta(data_path, reload=False):
    # Check if concatenated file already exists:
    if os.path.isfile(os.path.join(data_path, 'all_recordingMeta.pkl')) and not reload:
        df_recs = pd.read_pickle(os.path.join(data_path, 'all_recordingMeta.pkl'))
        return df_recs
    # Else read in the individual recordings, concatenate, and save them
    df_rec_dict = dict()
    recordings = [f'{i+1:02d}' for i in range(12)]
    for rec in recordings:
        # Recording meta data dataframes
        df_rec = pd.read_csv(os.path.join(data_path, f'{rec}_recordingMeta.csv'),
                             header=0,
                             index_col=False,
                             names=(
                                 'Recording ID',
    ...
    df_recs.to_pickle(os.path.join(data_path, 'all_recordingMeta.pkl'))
    return df_recs


# Track meta data
def read_track_meta(data_path, reload=False):
    if os.path.isfile(os.path.join(data_path, 'all_trackMeta.pkl')) and not reload:
        df_track_metas = pd.read_pickle(os.path.join(data_path, 'all_trackMeta.pkl'))
        return df_track_metas
    # Else read in the individual recordings, concatenate, and save them
    df_track_meta_dict = dict()
    track_metas = [f'{i+1:02d}' for i in range(12)]
    for tm in track_metas:
        # Track meta data dataframes
        df_track_meta = pd.read_csv(os.path.join(data_path, f'{tm}_tracksMeta.csv'),
                                    header=0,
                                    names=(
                                        'Vehicle ID',
                                        'Vehicle Length_Meta',
        ...
        # Add to dict
        df_track_meta_dict[tm] = df_track_meta
    # Concatenate and save
    df_track_metas = pd.concat([df_track_meta_dict[tm] for tm in track_metas], ignore_index=True)
    df_track_metas.to_pickle(os.path.join(data_path, 'all_trackMeta.pkl'))
    return df_track_metas
...
class HighD(Simulator):
    ...
    def __init__(self, **kwargs):
        ...
        self.track_meta = read_track_meta('traffic-data/xy-trajectories/highD/', reload=True)
        # kwargs['nb_lanes'] = max(self.rec_meta['Number Lanes'].values)  # take max number of lanes
        ...
        self._time_slots = [i+1 for i in range(12)]  # competition has data from 12 recordings
        self._t_slot = None
        ...
        self.nb_lanes = dict(zip(self.rec_meta['Recording ID'], self.rec_meta['Number Lanes']))
        self.lanes = self.build_lanes(self.nb_lanes)  # create dictionary of lane markings for each recording
        self.max_frame = -1
        ...
        lane_widths = np.array([ul[1] - ul[0] for ul in self.rec_meta['Upper Lane Markings'].values])
        self.lane_width = lane_widths.mean() * SCALE  # average distance between first and second lane markings

    def build_lanes(self, nb_lanes):
        """
        Build dictionary of lane markings with recording IDs (1 - 12) as keys
        :return: dictionary of lane markings with recording IDs (1 - 12) as keys
        """
        lane_markings = {}
        for i in range(12):
            upper_lanes = self.rec_meta[self.rec_meta['Recording ID'] ==
                                        i+1]["Upper Lane Markings"].values[0] * SCALE + Y_OFFSET
            lower_lanes = self.rec_meta[self.rec_meta['Recording ID'] ==
                                        i+1]["Lower Lane Markings"].values[0] * SCALE + Y_OFFSET
            lanes = {
                'upper_lanes': [
                    {
                        'min': upper_lanes[ul],
                        'mid': (upper_lanes[ul] + upper_lanes[ul + 1]) / 2,
                        'max': upper_lanes[ul + 1]
                    }
                    for ul in range(len(upper_lanes) - 1)
                ],
                'lower_lanes': [
                    {
                        'min': lower_lanes[ll],
                        'mid': (lower_lanes[ll] + lower_lanes[ll + 1]) / 2,
                        'max': lower_lanes[ll + 1]
                    }
                    for ll in range(len(lower_lanes) - 1)
                ]
            }
            lane_markings[f'{i+1}'] = lanes
        return lane_markings

    def _read_data_frame(self, data_path, reload=False):
        # Check if pkl file already exists:
        if os.path.isfile(os.path.join(data_path, 'observations.pkl')) and not reload:
            merged_tracks_df = pd.read_pickle(os.path.join(data_path, 'observations.pkl'))
            return merged_tracks_df
        # Track dataframes
        dtypes_dict = {
            'Recording ID': np.int64,
            'Frame ID': np.int64,
            'Vehicle ID': np.int64,
            'Local X': np.float64,
            'Local Y': np.float64,
        ...
        df_track = pd.read_csv(os.path.join(data_path, '00_observation.csv'),
                               header=0,
                               names=(
                                   'Recording ID'
                                   'Frame ID',
        ...
        # Re-index frames to 0  TODO: Do we need this for this dataset --> it starts at frame 125
        ...
        merged_rec_df = df_track.join(self.rec_meta[rec_cols].set_index('Recording ID'), on='Recording ID', how='left')

        # Merge tracks meta into tracks df on Vehicle ID key
        merged_tracks_df = merged_rec_df.join(self.track_meta.set_index('Vehicle ID'), on='Vehicle ID', how='left')
        ...
        # Save pkl file
        merged_tracks_df.to_pickle(os.path.join(data_path, f'observations.pkl'))
        return merged_tracks_df
        ...
        if vehicles:
            ...
            for vehicle_id in vehicles:
                ...
                if self.controlled_car and \
                    ...
                    car.lanes = self.lanes[df[df['Frame ID'] == self.frame]['Recording ID']]
    ...
    def _draw_lanes(self, surface, recording, mode='human', offset=0):
        lanes = self.lanes[f'{recording}']  # lanes
