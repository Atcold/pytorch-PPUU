...
class PCA:
    ...
    def __init__(self):
        ...
        if os.path.exists(PCA.SAVE_PCA_PATH):
            with open(PCA.SAVE_PCA_PATH, 'rb') as f:
                self.pca = pickle.load(f)
            with open(PCA.SAVE_KM_PATH, 'rb') as f:
            ...
            self.pca = sklearn.manifold.TSNE(n_components=2)
            self.pca.fit(features)
            self.km = sklearn.cluster.KMeans(n_clusters=5).fit(features)
            with open(PCA.SAVE_PCA_PATH, 'wb') as f:
                pickle.dump(self.pca, f)
            with open(PCA.SAVE_KM_PATH, 'wb') as f:
                pickle.dump(self.km, f)

    def transform(self, value):
        """ Performs dimensionality reduction on given values"""
        return self.pca.fit_transform(value)
    ...
    def get_model_failing_features(experiment, seed, checkpoint):
        ...
        for fail in failing:
            ...
            for i in range(len(features)):
                features[i] = np.pad(features[i], (history_size - features[i].shape[0], 0), 'constant')
            features = np.stack(features)
            features = features.flatten()
            l = costs[fail - 1]['collisions_per_frame'].shape[0]
            features = np.append(features, [costs[fail - 1]['collisions_per_frame'][l - 1]])
            features = np.append(features, [costs[fail - 1]['arrived_to_dst'][l - 1]])
            features = np.append(features, [states[fail - 1][l - 1][0]])
            features = np.append(features, [states[fail - 1][l - 1][1]])
            features = np.append(features, DataReader.get_last_gradient(experiment, seed, checkpoint, fail))
    ...
    def get_pca_data():
        ...
        for seed in seeds:
            ...
            for checkpoint in checkpoints:
                try:
                    if data == []:
                        data = PCA.get_model_failing_features(experiment, seed, checkpoint)
                    else:
                        data = np.concatenate([data, PCA.get_model_failing_features(experiment, seed, checkpoint)])
