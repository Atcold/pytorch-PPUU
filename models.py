...
class FwdCNN_TEN3(nn.Module):
    ...
    def create_policy_net(self, opt):
        if opt.targetprop == 0:
            ...
            if opt.actions_subsample == -1:
                context_dim = 0
            self.policy_net1 = StochasticPolicy(opt, context_dim=context_dim, output_dim=opt.n_actions)
            self.policy_net2 = StochasticPolicy(opt, context_dim=0, output_dim=opt.context_dim)
        elif opt.targetprop == 1:
            self.policy_net = PolicyMDN(opt)
    ...
    def train_policy_net(self, inputs, targets, targetprop=0):
        ...
        total_entropy = Variable(torch.zeros(1).cuda())
        for t in range(npred):
            if self.opt.actions_subsample == -1:
                actions_context = None
                actions, entropy, _, _ = self.policy_net1(input_images, input_states)
            else:
                if (t % self.opt.actions_subsample) == 0:
                    actions_context, entropy, _, _ = self.policy_net2(input_images, input_states)
                    total_entropy += entropy
                actions, entropy = self.policy_net1(input_images, input_states, context=actions_context)
            # encode the inputs
            total_entropy += entropy
            h_x = self.encoder(input_images, input_states)
            target_images, target_states, target_costs = targets
        ...
        pred_costs = torch.stack(pred_costs, 1)
        pred_actions = torch.stack(pred_actions, 1)
        return [pred_images, pred_states, pred_costs, entropy], pred_actions
