...
We also set the dimension of $z$ to be lower (2 dimensions) and pretrained the model deterministically with $z=0$.

Average results using the best-of-$k$ metric along with $95\%$ confidence intervals are shown in Figure \ref{bair}. Our method performs slightly worse on average than the model of \citep{Denton2018}, however this difference is not statistically significant. Our method and that of \citep{Denton2018} perform better than \citep{Babaeizadeh2018} according to the SSIM metric and worse according to PSNR.
Examples of video generations can be found at \url{url}.
