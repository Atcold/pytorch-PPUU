...
parser.add_argument('-model_dir', type=str, default='/misc/vlgscratch4/LecunGroup/nvidia-collab/models_images/')
...
def train(nbatches):
    ...
    for i in range(nbatches):
        optimizer.zero_grad()
        states, actions = dataloader.get_batch_il('train')
        states = Variable(states)
        actions = Variable(actions)
        pred_a, loss_kl = policy(states, actions)
...
def test(nbatches):
    ...
    total_loss_mse, total_loss_kl = 0, 0
    for i in range(nbatches):
        states, actions = dataloader.get_batch_il('valid')
        states = Variable(states)
        actions = Variable(actions)
        pred_a, loss_kl = policy(states, actions)
        loss_mse = F.mse_loss(pred_a, actions)
