...
def lane_cost(images, car_size):
    ...
    temp_c = (proximity_mask * images[:, :, 0].float()).view(bsize, npred, -1)
    costs = torch.sum(temp_c * F.softmax(temp_c, dim=-1), dim=-1)
...
def proximity_cost(images, states, car_size=(6.4, 14.3), green_channel=1, unnormalize=False, s_mean=None, s_std=None):
    ...
    temp_c = (proximity_mask * images[:, :, green_channel].float()).view(bsize, npred, -1)
    costs = torch.sum(temp_c * F.softmax(temp_c, dim=-1), dim=-1)
